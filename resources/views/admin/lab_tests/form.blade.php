...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-lg shadow-sm border border-blue-400 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-blue-100 bg-green">
            <h2 class="text-xl font-semibold text-green-600">{{ isset($labTest) ? 'Edit Lab Test' : 'Add New Lab Test' }}</h2>
            <p class="text-sm text-black-500 mt-1">Fill in the details below to {{ isset($labTest) ? 'update' : 'create' }} a lab test</p>
        </div>

        <!-- Form Content -->
        <form action="{{ isset($labTest) ? route('admin.lab-tests.update', $labTest->id) : route('admin.lab-tests.store') }}" method="POST" class="p-6">
            @csrf
            @if(isset($labTest))
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $labTest->name ?? '') }}" class="block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('name')
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="number" step="0.01" min="0" name="price" value="{{ old('price', $labTest->price ?? '') }}" class="block w-full pl-8 rounded-md border-blue-200 focus:border-blue-500 focus:ring-blue-500" required>
                    </div>
                    @error('price')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative rounded-md shadow-sm">
                        <input type="text" name="turnaround_time" value="{{ old('turnaround_time', $labTest->turnaround_time ?? '') }}" class="block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="e.g. 24-48 hours">
                    </div>
                </div>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $labTest->description ?? '') }}</textarea>
                </div>

                <!-- Test Requirements -->
                <div class="space-y-2 md:col-span-2">
                    <label class="block text-sm font-medium text-black-600">Test Requirements</label>
                    <textarea name="test_requirements" rows="3" class="w-full border px-3 py-2 rounded">{{ old('test_requirements', $labTest->test_requirements ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="mt-8 flex justify-end space-x-3">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-black bg-green-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    {{ isset($labTest) ? 'Update Lab Test' : 'Save Lab Test' }}
                </button>
            </div>
        </form>
    </div>
