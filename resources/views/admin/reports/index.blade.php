...
<td class="px-6 py-4 whitespace-nowrap">
    @php
        ...
        $currentStatus = $report->status; // Uses either database field or virtual attribute
    @endphp
    
    <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $statusColors[$currentStatus] ?? 'bg-gray-100 text-gray-800' }}">
...
<td class="px-6 py-4 whitespace-normal text-sm text-gray-900 max-w-xs">
    @if($report->result)
        <div class="whitespace-pre-wrap">{!! $report->formatted_result !!}</div>
    @else
